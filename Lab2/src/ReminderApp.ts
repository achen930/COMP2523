...
export default class ReminderApp {
  ...
  private handleToggleCompletion(): void {
    if (this._remindersHandler.size() === 0) {
      ...
      const reminderNumber = Number(this.getUserChoice("index", false));
      this._remindersHandler.toggleCompletion(reminderNumber);
  ...
  private handleModifyReminders(): void {
    if (this._remindersHandler.size() === 0) {
      ...
      Logger.logReminders(this._remindersHandler.reminders);
      const reminderNumber = Number(this.getUserChoice("index", false));
      const modifiedReminder = this.getUserChoice("reminder", false);
      this._remindersHandler.modifyReminder(reminderNumber, modifiedReminder);
      if (ReminderApp.checkUserToggleChoice() === true) {
        this._remindersHandler.toggleCompletion(reminderNumber);
      }
      Logger.log('\n  🏁   Reminder Modified');
    }
  ...
  private handleAddReminder(): void {
    const reminderInput = this.getUserChoice("reminder", false);
    const tagInput = this.getUserChoice("tag", false);
    this._remindersHandler.addReminder(reminderInput, tagInput);
    Logger.log('\n  🏁  Reminder Added')
  }
  ...
  private handleSearchReminders(): void {
    if (this._remindersHandler.size() === 0) {
      ...
      const keyword = this.getUserChoice("keyword", false);
